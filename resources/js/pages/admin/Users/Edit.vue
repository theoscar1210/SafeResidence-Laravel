    ...
    <AppLayout>
        ...
        <div class="mx-auto max-w-2xl p-6">
            <div class="mb-6 flex items-center gap-4">
                <Link href="/admin/users" class="text-muted-foreground hover:text-foreground">← Volver</Link>
                <div>
                    <h1 class="text-2xl font-bold">Editar Usuario</h1>
                    <p class="text-sm text-muted-foreground">{{ user.first_name }} {{ user.last_name }}</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-5 rounded-xl border bg-card p-6 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="grid gap-1.5">
                        ...
                        <p v-if="form.errors.cedula" class="text-xs text-destructive">{{ form.errors.cedula }}</p>
                    </div>
                    <div class="grid gap-1.5">
                ...
                <div class="grid gap-1.5">
                    <Label>Contraseña <span class="text-muted-foreground">(dejar en blanco para no cambiar)</span></Label>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="grid gap-1.5">
                            <Input id="password" type="password" v-model="form.password" placeholder="Nueva contraseña" />
                            <p v-if="form.errors.password" class="text-xs text-destructive">{{ form.errors.password }}</p>
                        </div>
                        <div class="grid gap-1.5">
                            <Input id="password_confirmation" type="password" v-model="form.password_confirmation" placeholder="Confirmar contraseña" />
                        </div>
                    </div>
                </div>
