        ...
        <div class="flex flex-col gap-6 p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-xl border bg-card p-5 shadow-sm">
                    ...
                    <p class="text-4xl font-bold text-primary">{{ stats.authorizations_active }}</p>
                </div>
                <div class="rounded-xl border bg-card p-5 shadow-sm">
                    <p class="text-sm text-muted-foreground">Total autorizaciones</p>
                    <p class="text-4xl font-bold">{{ stats.authorizations_total }}</p>
                </div>
                <div class="rounded-xl border bg-card p-5 shadow-sm">
                    <p class="text-sm text-muted-foreground">Personas en edificio</p>
                    <p class="text-4xl font-bold">{{ stats.inside_now }}</p>
                    <p class="mt-1 text-xs text-muted-foreground">relacionadas a tu apartamento</p>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-muted/50">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left font-medium">Vigencia</th>
                            <th class="px-5 py-3 text-left font-medium">Estado</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="a in authorizations" :key="a.id" class="border-t hover:bg-muted/30">
                            <td class="px-5 py-3 font-medium">{{ a.full_name }}</td>
                            <td class="px-5 py-3">{{ a.cedula }}</td>
                            <td class="px-5 py-3">
                                <span :class="['rounded-full px-2 py-0.5 text-xs font-medium', typeClass[a.type] ?? 'bg-muted text-muted-foreground']">
                                    {{ a.type }}
                                </span>
                            </td>
                            <td class="px-5 py-3 text-xs">{{ a.start_date }} → {{ a.end_date }}</td>
                            <td class="px-5 py-3">
                                <span :class="['rounded-full px-2 py-0.5 text-xs font-medium', statusClass[a.status] ?? 'bg-muted text-muted-foreground']">
                                    {{ a.status }}
                                </span>
                            </td>
                        </tr>
                        <tr v-if="authorizations.length === 0">
                            <td colspan="5" class="px-5 py-8 text-center text-muted-foreground">
                                No tienes autorizaciones. <Link href="/propietario/authorizations/create" class="text-primary underline">Crea una ahora</Link>.
                            </td>
                        </tr>
                    </tbody>
                </table>
