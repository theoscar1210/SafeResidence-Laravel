...
watch(
    ...
    () => {
        ...
        filterTimeout = setTimeout(() => {
            router.get('/residente/history', localFilters.value, {
                preserveState: true,
                replace: true,
            });
        }, 400);
        ...
        <div class="flex flex-col gap-6 p-4 sm:p-6">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Accesos registrados
                    <span v-if="apartment">
                        al inmueble <strong>{{ apartment }}</strong></span
                    >
            ...
            <div class="grid gap-3 sm:flex sm:flex-wrap sm:items-end">
                ...
                <div class="grid gap-1">
                    ...
                    <Input v-model="localFilters.date_from" type="date" class="w-full sm:w-40" />
                </div>
                <div class="grid gap-1">
                    <label class="text-xs font-medium text-muted-foreground">Hasta</label>
                    <Input v-model="localFilters.date_to" type="date" class="w-full sm:w-40" />
                </div>
            </div>
                        ...
                        >
                            ...
                            <td class="px-4 py-3">
                                <span v-if="entry.is_inside" class="inline-flex items-center gap-1.5 text-xs font-semibold text-green-600">
                                    <span class="h-2 w-2 rounded-full bg-green-500"></span>Dentro
                                </span>
                                <span v-else class="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                                    <span class="h-2 w-2 rounded-full bg-slate-300"></span>Salió
                                </span>
                            </td>
                        </tr>
            ...
            >
                <p class="text-muted-foreground">
                    Mostrando {{ entries.from }}–{{ entries.to }} de {{ entries.total }} registros
                </p>
